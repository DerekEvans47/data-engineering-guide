...
(async () => {
  ...
  // Boot renders the home screen after loadWorldData() resolves.
  ...
  // The standalone game must not carry any learning UI.
  const learnCard = await page.$('#btn-learn');
  if (learnCard) throw new Error('Learn card present — game should be game-only');
  console.log('✅ Home is game-only (no Study/Drill card)');
