...
| G-9 | Multi-path map topology + fixed build-slot placement | 75 | P1 | TODO | — | Current maps are architecturally single-lane: `tdComputePathSet(wps)` connects one flat waypoint array into one axis-aligned route, and tower placement accepts *any* non-path cell (`!td.pathSet.has(...) && !td.towers.some(...) && gold >= cost`) — always predictable, no tower-coverage tradeoffs. Redesign toward Kingdom-Rush-style maps with 2+ concurrent lanes AND a curated, deliberately scarce list of fixed build slots per map (not free placement on any open cell) — the two are one redesign, not two, because hand-placed slots are what actually *guarantees* "no single tower placement covers both paths" rather than hoping procedural placement happens to produce that tension. Needs: (1) path data model generalized from one `wps` array to multiple named lanes with per-lane `pathSet`s (spawn/movement code tags each enemy with a lane index); (2) audit every function assuming a single path — `tdMoveEnemy`, tower targeting/range checks, `tdGenerateTerrainDeco` (union pathSets before deco placement), wave spawn distribution across lanes; (3) new `buildSlots: [[col,row],...]` field per level, and placement validation switched from "any non-path cell" to "cell is in this level's slot list and unoccupied"; (4) new hand-crafted multi-lane map layouts (V-11 built 9 single-lane maps — these need redesigning or replacing, not just new waypoint lists on the old model) — this pairs naturally with the V-34/V-35 follow-on painted-background art (option C from that exploration), since a hand-painted background needs its clearings and its slot list hand-matched to each other anyway; slot layout can't be generic/reusable across a theme the way the current flat-terrain system is. Doesn't have to be all-or-nothing — could apply to a handful of "signature" maps first while others stay on the current procedural free-placement system. Reference images: Kingdom Rush's branching-river maps with distinct build platforms vs. this project's current single serpentine lane with placement anywhere. |
