...
| G-9 | Multi-path map topology + fixed build-slot placement + landscape orientation (battle map AND run map) | 85 | P1 | TODO | — | **⚠️ Do not auto-implement — requires the user directly, see `ART_DIRECTION_HANDOFF.md`.** This item depends on hand-picked painted background art that does not exist in the repo yet (all exploration so far happened as chat-shared images, none saved to disk) and on art-direction/lore decisions still being iterated conversationally with the user — a from-scratch autonomous implementation working only from this row's prose would very likely diverge from what's actually been decided. Also touches the same function as the separate "Diagonal path geometry" idea in the GitHub Pages Feasibility Notes below (`tdComputePathSet`) — fold that in here rather than filing it separately, to avoid two uncoordinated redesigns of the same function. Current maps are architecturally single-lane: `tdComputePathSet(wps)` connects one flat waypoint array into one axis-aligned route, and tower placement accepts *any* non-path cell (`!td.pathSet.has(...) && !td.towers.some(...) && gold >= cost`) — always predictable, no tower-coverage tradeoffs. Redesign toward Kingdom-Rush-style maps with 2+ concurrent lanes AND a curated, deliberately scarce list of fixed build slots per map (not free placement on any open cell) — the two are one redesign, not two, because hand-placed slots are what actually *guarantees* "no single tower placement covers both paths" rather than hoping procedural placement happens to produce that tension. Also switches the battle-map play grid from portrait (`TD_COLS=9, TD_ROWS=10`) to landscape, targeting roughly 16:9 (exact dimensions TBD during implementation) — confirmed low-risk: `td.cellSize = Math.min(cellByW, cellByH)` already responsively fits the canvas to any grid shape, and every one of the ~20 `TD_COLS`/`TD_ROWS` call sites (path bounds, deco loops, stereo audio pan, placement-chip UI offset) is already written generically against the constants, not hardcoded to current values. **Correction from initial scoping:** the run map (`renderRunMap`, `#tdm-svg`, currently a portrait `340x540` viewBox) must ALSO switch to landscape, matching the battle map — initially scoped as "stays portrait, it's just a menu," but that's wrong once transition frequency is considered: the player alternates between run map and battle map constantly within one play session (pick node → play → back to run map → pick next node → ...), so mismatched orientations mean rotating the device on every single transition, not once. The portrait-is-natural-for-menus reasoning only applies to the outer study-guide site (reading guide content, browsing parts/questions), which isn't interleaved with battle-map gameplay in a tight loop — a session rotates to landscape once at the start of play and stays there through run map ↔ battle map ↔ run map until the player exits back to the guide. Needs: (1) path data model generalized from one `wps` array to multiple named lanes with per-lane `pathSet`s (spawn/movement code tags each enemy with a lane index); (2) audit every function assuming a single path — `tdMoveEnemy`, tower targeting/range checks, `tdGenerateTerrainDeco` (union pathSets before deco placement), wave spawn distribution across lanes; (3) new `buildSlots: [[col,row],...]` field per level, and placement validation switched from "any non-path cell" to "cell is in this level's slot list and unoccupied"; (4) new landscape-dimension hand-crafted multi-lane map layouts (V-11's 9 maps are portrait-authored and need redoing regardless of orientation, so the dimension change adds no incremental map-authoring cost); (5) `#tdm-svg` viewBox widened to landscape to match, node/road layout logic in `renderRunMap` re-tuned for the new proportions; (6) new mobile orientation handling — nothing exists today (no `orientation` media query, no rotate-device prompt) — needs a simple portrait-detection overlay prompting landscape, applied once per play session at the run-map/battle-map boundary, not per-screen. Pairs naturally with the V-34/V-35 follow-on painted-background art (option C from that exploration) for both the battle-map terrain and the run-map's parchment-style background, since both need bespoke art matched to their specific layout rather than generic reusable tiles. Doesn't have to be all-or-nothing — could apply to a handful of "signature" maps first while others stay on the current portrait procedural free-placement system. Reference images: Kingdom Rush's branching-river landscape maps with distinct build platforms vs. this project's current portrait single serpentine lane with placement anywhere. |
...
- **Diagonal path geometry** (not yet filed): Would require changes to `tdComputePathSet` and `tdMoveEnemy`, which currently only handle axis-aligned segments. Medium complexity. **G-9 already redesigns `tdComputePathSet` for multi-lane support — fold this into G-9's implementation rather than filing it as a separate item**, to avoid two uncoordinated redesigns of the same function.
