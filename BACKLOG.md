...
| G-9 | Multi-path map topology (branching/parallel lanes) | 60 | P1 | TODO | — | Current maps are architecturally single-lane: `tdComputePathSet(wps)` connects one flat waypoint array into one axis-aligned route — always predictable, no tower-coverage tradeoffs. Redesign toward Kingdom-Rush-style maps with 2+ concurrent lanes (parallel routes and/or fork points enemies are distributed across) so no single tower placement covers everything. Needs: (1) path data model generalized from one `wps` array to multiple named lanes with per-lane `pathSet`s (spawn/movement code tags each enemy with a lane index); (2) audit every function assuming a single path — `tdMoveEnemy`, tower targeting/range checks, `tdGenerateTerrainDeco` (union pathSets before deco placement), wave spawn distribution across lanes; (3) new hand-crafted multi-lane map layouts (V-11 built 9 single-lane maps — these need redesigning or replacing, not just new waypoint lists on the old model). Largest single engine change under discussion; doesn't require new art and is independent of the pixel-art-style decision, so it can land before or after V-34/V-35 asset work. Reference images: Kingdom Rush's branching-river maps vs. this project's current single serpentine lane. |
