...
| V-36 | Painted-pixel-art tower sprite sheets (match battle-map art style) | 45 | P1 | TODO | V-16 | Current towers (`TD_SPRITES.bastion/ranger/mortar`) are flat hand-authored pixel-art frames recolored per tier (`pals[lvl]`, shipped as V-16) — same rendering technique as the old procedural terrain, which reads as a "sticker" pasted on top of the new painted battle-map backgrounds (frontier-town.png and successors). Fix: regenerate each tower as a painted-pixel-art sprite sheet in the same Nano Banana pipeline as the battle maps, camera/style-anchored to `frontier-town.png`, with a baked-in grounded shadow so it reads as standing IN the scene rather than layered over it. Prompt book: `TOWER_GENERATION_PROMPTS.md` (Ranger done first). Silhouette continuity across upgrade tiers is the hard requirement — a prior 4×4 feasibility test failed this exact check (tier 1 read as a different building from tiers 2/3), see `ART_DIRECTION_HANDOFF.md` item 4. Once art lands: write a magenta chroma-key slicer (simpler than `remove_checker_bg.py`, which is checkerboard-specific — a flat-color threshold suffices here), wire sliced frames into `TD_SPRITES`, retire the old hand-authored pixel frames for whichever tower(s) got new art. Bastion and Mortar follow the same pipeline once Ranger's approach is validated. |
