...
| ID  | Title | Effort | Priority | Status | Dependencies | Notes |
|-----|-------|--------|----------|--------|--------------|-------|
| W-1 | Linear world-map spine on painted map art | 55 | P1 | TODO | — | **⚠️ Do not auto-implement — needs the user's chosen map + hand-placed node coordinates.** Replaces `generateRun`'s 3-lane graph and `RUN_NODE_POS` with a per-preset ordered node table: a spine of 12–16 nodes (≥8 battles, boss last) hand-placed along the painted trail of the chosen map image. Landscape SVG viewBox sized to the map image; node markers overlaid at runtime from the `verdant-worldmap-markers.png` catalog (node types still shuffle per run within the fixed spine); fog-of-war + mist-creep hooks reserved for W-3. Three candidate Verdant maps were generated 2026-07-02 (baseline + Golden Valley + Wild Frontier) — user picks and commits the winner(s); multiple presets per world remain the plan (3 per difficulty eventually). Shares the once-per-session landscape-orientation prompt with G-9 item (6). |
| W-2 | Battle engagement tiers (standard vs elite contract) | 30 | P1 | TODO | W-1 | Pre-battle stakes choice on every battle node: **Standard** (normal waves/questions/gold) or **Elite contract** (harder waves + harder question tier + constrained build slots → 2–3× gold, relic-drop chance, bonus XP). Reuses existing `diffWeights`/`getDiffInfo` tiers and EQ-4 relics as the premium reward. Elite stops being a random node type and becomes an opt-in mode on any battle — for a study app this aims the incentive at harder question material. |
