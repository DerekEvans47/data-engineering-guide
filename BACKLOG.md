...
| G-9 | Multi-path map topology + fixed build-slot placement + landscape orientation | 80 | P1 | TODO | — | Current maps are architecturally single-lane: `tdComputePathSet(wps)` connects one flat waypoint array into one axis-aligned route, and tower placement accepts *any* non-path cell (`!td.pathSet.has(...) && !td.towers.some(...) && gold >= cost`) — always predictable, no tower-coverage tradeoffs. Redesign toward Kingdom-Rush-style maps with 2+ concurrent lanes AND a curated, deliberately scarce list of fixed build slots per map (not free placement on any open cell) — the two are one redesign, not two, because hand-placed slots are what actually *guarantees* "no single tower placement covers both paths" rather than hoping procedural placement happens to produce that tension. Also switches the play grid from portrait (`TD_COLS=9, TD_ROWS=10`) to landscape, targeting roughly 16:9 (exact dimensions TBD during implementation) — confirmed low-risk: `td.cellSize = Math.min(cellByW, cellByH)` already responsively fits the canvas to any grid shape, and every one of the ~20 `TD_COLS`/`TD_ROWS` call sites (path bounds, deco loops, stereo audio pan, placement-chip UI offset) is already written generically against the constants, not hardcoded to current values. Needs: (1) path data model generalized from one `wps` array to multiple named lanes with per-lane `pathSet`s (spawn/movement code tags each enemy with a lane index); (2) audit every function assuming a single path — `tdMoveEnemy`, tower targeting/range checks, `tdGenerateTerrainDeco` (union pathSets before deco placement), wave spawn distribution across lanes; (3) new `buildSlots: [[col,row],...]` field per level, and placement validation switched from "any non-path cell" to "cell is in this level's slot list and unoccupied"; (4) new landscape-dimension hand-crafted multi-lane map layouts (V-11's 9 maps are portrait-authored and need redoing regardless of orientation, so the dimension change adds no incremental map-authoring cost); (5) new mobile orientation handling — nothing exists today (no `orientation` media query, no rotate-device prompt) — needs a simple portrait-detection overlay prompting landscape, similar in spirit to the existing `prefers-reduced-motion` adaptive pattern (U-5). Pairs naturally with the V-34/V-35 follow-on painted-background art (option C from that exploration), since a hand-painted background's clearings and its slot list need to be hand-matched to each other anyway; slot layout can't be generic/reusable across a theme the way the current flat-terrain system is. Doesn't have to be all-or-nothing — could apply to a handful of "signature" maps first while others stay on the current portrait procedural free-placement system. Reference images: Kingdom Rush's branching-river landscape maps with distinct build platforms vs. this project's current portrait single serpentine lane with placement anywhere. |
