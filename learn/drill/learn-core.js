'use strict';
// Data Engineering — Study & Drill: a standalone learning app (Study Mode,
// Drill/flashcards, Daily Challenge) built on the shared question bank. Storage,
// XP/streaks/achievements, boot, home/profile/filter UI, and the study/drill
// question flow. No tower-defense game code — this app is fully independent.
//
// Single classic script sharing the global scope, loaded by index.html.
...
(async () => {
  ...
  try {
    ...
    const hit = keys.find(k => k.startsWith('de-study-'));
    if (!hit) return;
    APP_VERSION = hit.replace('de-study-v', '');
...
// Gamification state (XP / streak / achievements)
let xp           = 0;
let streak       = 0;
let bestStreak   = 0;
...
// Cached DOM refs — populated in bindUI() to avoid repeated getElementById calls.
const EL = {};
...
document.addEventListener('DOMContentLoaded', async () => {
  // The question bank is this app's one required dependency.
  try {
...
// ── Top bar ────────────────────────────────────────────────────
// The home screen builds its own header, so the persistent top bar is hidden
// there and only shown on the study/drill screens.
const HIDDEN_TOPBAR_STATES = new Set(['home']);

function setTopBar(state, extra = {}) {
  const bar = document.getElementById('top-bar');
  if (HIDDEN_TOPBAR_STATES.has(state)) {
...
function openProfile() {
  ...
  const masteredCount = Object.values(mastery).filter(m => (m.correct ?? 0) >= MASTERY_THRESHOLD).length;
  const seenCount     = new Set(JSON.parse(StorageManager.get(SEEN_KEY) || '[]')).size;
  ...
  sheet.innerHTML = `
    ...
    <div class="profile-panel">
      ...
      <div class="profile-stats">
        ...
        <div class="stat-cell"><div class="stat-val">⭐ ${masteredCount}</div><div class="stat-label">Mastered</div></div>
        <div class="stat-cell"><div class="stat-val">${seenCount}</div><div class="stat-label">Seen in Drill</div></div>
      </div>
      <div class="ach-grid-title">Achievements</div>
...
function exportSave() {
  const EXPORT_KEYS = [
    ...
    ACHIEVEMENTS_KEY, DAILY_KEY, ACCURACY_KEY, MASTERY_KEY,
  ];
  ...
  a.href     = url;
  a.download = 'study-drill-save.json';
  a.click();
...
function showHome() {
  ...
  EL.contentArea.innerHTML = `
    <div class="home-screen">
      ...
      <div class="home-hero">
        ...
        <span class="home-subtitle">Data Engineering · ${totalQ} questions</span>
      </div>
      <div class="home-bottom">
        <div class="home-stats">
          <div class="home-stat" id="home-stat-xp">⚡ ${xp.toLocaleString()} XP</div>
          <div class="home-stat">🔥 ${streak} streak</div>
          <div class="home-stat">${l.icon} ${l.title}</div>
        </div>
        <div class="home-menu">
          <button class="home-card home-card-primary" id="btn-study">
            <span class="home-card-icon">📚</span>
            <div class="home-card-text">
              <span class="home-card-title">Study by Part</span>
              <span class="home-card-desc">All 9 parts · progress tracked</span>
            </div>
            <span class="home-card-arrow">›</span>
          </button>
          <button class="home-card home-card-secondary" id="btn-drill">
            <span class="home-card-icon">⚡</span>
            <div class="home-card-text">
              <span class="home-card-title">Drill Mode</span>
              <span class="home-card-desc">Randomized flashcards across all parts</span>
            </div>
            <span class="home-card-arrow">›</span>
          </button>
          ${!daily.complete ? `
          <button class="home-card home-card-secondary" id="btn-daily-home">
            <span class="home-card-icon">📅</span>
            <div class="home-card-text">
              <span class="home-card-title">Daily Challenge</span>
              <span class="home-card-desc">${dqs.length} questions · 1.5× XP</span>
            </div>
            <span class="home-card-arrow">›</span>
          </button>` : ''}
        </div>
  ...
  document.getElementById('btn-study').addEventListener('click', () => {
    mode = 'study'; EL.app.dataset.mode = 'study';
    setTopBar('study-list'); showPartList();
  });
  document.getElementById('btn-drill').addEventListener('click', () => {
    mode = 'drill'; EL.app.dataset.mode = 'drill';
    setTopBar('drill'); buildDrillQueue(); showNext();
  });
  ...
  // Force-update escape hatch: tapping the version badge clears the service
