...
| I-7 | Split `learn/drill/` into a private repo (protect sellable game IP) | 15 | P1 | TODO | — | **⚠️ Do not auto-implement — needs the user to actually perform the split; targeted for later this week (decided 2026-07-21).** Licensing groundwork landed same day: root `LICENSE` (CC BY-NC 4.0, covers the guide + question bank) and `learn/drill/LICENSE` (all-rights-reserved, explicit carve-out) are both in place as an interim measure while the game and guide still share one repo. The actual split still needs: (1) move `learn/drill/` to a new private repo; (2) decide whether a public web build of the game keeps existing at all — note from the 2026-07-21 discussion: even with a private *source* repo, a live GitHub Pages site unavoidably ships all its JS/JSON/PNG in the clear to any visitor's browser (view-source/Network tab), so privatizing the repo alone does not protect a still-publicly-served build; real protection means distributing only as a built app-store binary (Capacitor/Cordova) rather than a public URL; (3) if a free web version is still wanted as a portfolio/demo piece, accept that page remains fully copyable regardless of repo visibility — explicit tradeoff, not a bug; (4) update root `README.md`'s repository-structure section once the directory is actually gone from this repo, and confirm GitHub Pages for the guide still builds fine without it. |
...
| G-3 | Tower special abilities (active skills, cooldowns) | 50 | P2 | SUPERSEDED | — | **2026-07-05:** absorbed by K-4 (quiz-charged active abilities) — same feature with a charge mechanic that fits the study loop better than wall-clock cooldowns. Marked so the nightly picker doesn't build the cooldown version. |
...
| G-12 | Rename lives/health → "Confidence" (thematic reskin) | 15 | P1 | TODO | — | **⚠️ Do not auto-implement — needs the user to decide the presentation before building.** Reskins the existing `td.lives`/`td.maxLives` stat (currently ❤️, integer countdown from `levelDef.startLives`, drains via per-enemy `lifeLoss` in `config.json`, game-over at 0) as "Confidence" — fits the study-app framing better than a generic health bar. Mechanic stays identical (same drain-on-leak, same game-over-at-zero); this is presentation only. Two open framing options to decide between: (1) pure reskin — same integer HUD, just relabeled + new icon (🧠 or similar); (2) percentage-gauge framing — display 0–100% instead of a raw integer, reads more naturally as "confidence" and leaves room for a later follow-up item (not in scope here) where correct quiz answers restore a sliver of confidence, tying the stat back into the actual study mechanic. Touches: `drill-td.js` (HUD labels/ids currently `td-lives`/`td-lives-fill`/`td-stat-lives`), `drill.css` (`.td-lives-bar` styling), any copy referencing "lives"/"health" in UI strings. |
| G-13 | Population — new resource, leak-linked, drives economy | 35 | P1 | TODO | G-12 | **⚠️ Do not auto-implement — needs the user to decide the economy hookup and end-state before building.** New third resource distinct from Confidence (G-12) and gold: enemies that leak past cost the player both Confidence *and* Population (a sibling `popLoss` field alongside the existing per-enemy `lifeLoss` in `config.json`). Population doesn't end the run directly — instead it scales the base economy, so heavy leaking snowballs into a weaker economy for the rest of the run (distinct stakes from Confidence hitting 0). Open decisions before implementation: (1) **which economy lever it drives** — leading candidate is a multiplier on the existing per-wave gold bonus (`waveGoldAdd`/`interestRate` pattern already in `drill-td.js` around the wave-clear gold calc), cheapest to wire in; alternatives are a build/upgrade cost modifier (recession/boom) or gating extra build slots (bigger scope, needs new UI) — pick one, don't build all three; (2) **floor behavior** — does population hitting 0 do anything (permanent economy floor, run-ending state) or is it a pure decaying multiplier with no bottom-out event; (3) **regen** — monotonically decreasing all run (pure scarcity pressure) vs. small trickle-back on clean (no-leak) waves. Depends on G-12 only in that both land together as one HUD/config pass, not a hard technical dependency. |
...
| W-7 | Ambient world-map life layer (animated overlay) | 30 | P2 | DONE | W-1 | **Shipped 2026-07-05** as `RVM_AMBIENT`/`rvmAmbientHtml` (CSS-animated SVG, reduced-motion gated): drifting gradient-lobe mist, wind-bent chimney smoke anchored to painted plumes, ripple-squiggle water glints, flock birds, pennant flags on tower tips, grazing sheep tinted to the painted herd. Cloud-shadow pass (idea 3) not built — the only unbuilt idea from the original list. Original spec: | The painted map stays a static PNG; life comes from a thin animated overlay at hand-authored coordinates (authored in the same pass as the W-1 node spine, stored per preset). Effects ranked by payoff/cost: (1) chimney/charcoal-camp smoke — drifting fading puffs from marked points (the art already paints static wisps at the charcoal camp to anchor them); (2) water glints — twinkling sparkles along marked river/lake polylines + slow drifting highlight dashes along flow direction (direct reuse of the V-13 flow-dot technique); (3) cloud shadows — 2-3 huge soft translucent shade blobs drifting slowly across the whole map; this is the answer to "tree sway": painted trees cannot be deformed without artifacts, but moving light over the canopy reads as wind; (4) corrupted-corner mist drift — slow circulating grey wisps at the keep corner, and the foundation the W-3 mist-creep visual extends along the road; (5) lantern glow-pulse at the waystation/hamlets (applyDecoAnimation glow-pulse pattern already exists); (6) rare tiny bird pairs drifting across (~every 20-30 s). All effects gated behind prefers-reduced-motion (U-5 pattern exists). Perf budget trivial: ~a dozen particles + 3 shadow blobs. |

---

## K — Genre Benchmarks (Kingdom Rush & standout TD study, 2026-07-05)

**Evaluation.** Reviewed against Kingdom Rush (Ironhide), Bloons TD6, GemCraft, and Defense
Grid. What the genre leaders do that we already match: painted world map with a linear
node journey (KR's signature frame — ours since W-1), per-node themed battle scenes (G-9
direction), radial build menu, wave preview, endless mode, difficulty toggles, gold economy
with carry-over. What they do that we **lack**, ranked by how much it would move our game:
(1) **pacing agency** — KR's early-call bonus and every TD's speed toggle are why waves never
feel slow; we have neither. (2) **moment-to-moment agency** — KR's Rain of Fire/Reinforcements
give the player something to DO mid-wave besides watch; our quiz gate is the natural charge
mechanic for this and would make "study harder → fight better" literal. (3) **build-choice
tension** — KR's armor/magic-resist matrix and tier-4 specialization branches force mixed,
deliberate builds; our 3 towers are interchangeable damage dealers today. (4) **tactical
micro** — BTD6's targeting priorities. (5) **personality** — named minibosses, intro banners,
barks; KR's humor is half its brand and it's nearly free. Everything below is static-Pages
compatible.

| ID  | Title | Effort | Priority | Status | Dependencies | Notes |
|-----|-------|--------|----------|--------|--------------|-------|
| K-1 | Game speed toggle (1×/2×) | 12 | P1 | TODO | — | Table stakes in every standout TD. HUD chip next to wave button; multiply `dt` in the update loop (NOT frame skipping — projectile hit tests already handle large steps). Persist choice per player. Also speeds quiz-disabled testing sessions. |
| K-2 | Early-call wave bonus (KR pattern) | 18 | P1 | TODO | — | Between waves, show an incoming banner with a ~10 s countdown; starting early grants bonus gold scaled by seconds skipped. Quiz twist: a CORRECT pre-wave answer doubles the early-call bonus — pace and mastery reinforce each other. Rewires the existing wave-preview card rather than adding a new surface. |
| K-3 | Per-tower targeting priority (First/Last/Strong/Weak) | 20 | P1 | TODO | — | BTD6's biggest cheap win. A 4-state toggle on the tower's radial/stat card; store per tower; `tdFireTowers` swaps its sort key. Default First. Matters most once G-2 special types mix into one wave. |
| K-4 | Active abilities charged by quiz correctness | 45 | P2 | TODO | G-3 absorbed | Supersedes G-3's cooldown framing. Two abilities on the battle HUD — an AoE strike (KR Rain of Fire) and a path slow/reinforcement — charged by CORRECT ANSWERS, not time. Wrong answers add nothing. This is our identity feature: knowledge is literally ammunition. Ability UI, charge meter, canvas FX. |
| K-5 | Tier-4 tower specialization branches | 50 | P2 | TODO | V-36 | KR-style: at tier 4 each tower picks one of two ultimates (e.g. Ranger → Sniper's Roost: huge range + armor pierce, vs Hail of Arrows: multishot + light AoE). Doubles endgame build variety without new base towers. Needs 2 new painted tier sheets per tower (TOWER_GENERATION_PROMPTS pipeline). |
| K-6 | Damage-type matrix (physical/magic vs armor/resist) | 35 | P2 | TODO | — | KR's build-tension engine. Two damage types on towers, two resist stats on enemies, icons on enemy HP bars; brute rework folds in. Forces mixed builds instead of stacking one tower. Balance pass on all existing defs included. |
| K-7 | Named minibosses with intro banner + bark | 15 | P2 | TODO | — | Every 3rd-ish node's final wave leads with a NAMED variant ("Grukk the Toll-Taker") — stat-multiplied existing enemy, banner slide-in, one flavor bark. KR-grade personality at trivial engine cost; pairs with the bandit_boss `isBoss` plumbing from G-11. |
| K-8 | Interest on banked gold between waves | 10 | P3 | TODO | EQ-1 | Defense Grid pattern: +5% (capped) on unspent gold at each wave start rewards restraint vs tower-spam. **Flag: interacts with EQ-3/EQ-5 gold items — balance together**, and keep the cap tight so it never outearns playing well. |
| K-9 | Per-node star ratings on the region map + heroic replays | 25 | P2 | TODO | — | Surface the old 3★ system per spine node (3★ = zero leaks) as small stars under each completed node marker; cleared nodes unlock a replayable "heroic" variant (fixed harder wave set, G-8 modifiers pre-locked) for stars that feed P-1's tree. KR's replay-depth loop. |
| K-10 | Hero/champion unit | 65 | P3 | TODO | G-10 | KR heroes are beloved but expensive: a controllable unit with level-ups needs the G-10 friendly-unit engine (path-walking, melee AI, respawn) first. Do not start before G-10 ships; revisit priority after it does. |
| K-11 | Battle intro flavor text per node | 8 | P3 | TODO | — | One lore line on the level-confirm panel per battleTheme ("The millstone hasn't turned since the bandits came."). 14 lines of writing + one panel slot; makes the painted world read as a place, not a level list. |
...
| W-1 | Linear world-map spine on painted region art (user-targeted grid-cell placement) | 55 | 2026-07-05 |
| W-7 | Ambient world-map life layer (mist/smoke/glints/flags/sheep/birds) | 30 | 2026-07-05 |
| — | Playtest round 4: landscape home fit, arrow physics rebalance, full-bleed map | — | 2026-07-05 |
...
| ID  | Title | Effort | Priority | Status | Dependencies | Notes |
|-----|-------|--------|----------|--------|--------------|-------|
| A-1 | Enemy sheet template + goblin pilot | 20 | P1 | DONE | — | **User + Nano.** Pilot complete 2026-07-04. **Key finding that rewrites the template: Nano's ceiling is TWO distinct animation keyframes per generation.** A 6-frame walk request produced 8 near-identical poses (verified by silhouette diffing); a restructured request for 4 individually-described, maximally-contrasted poses produced 2 genuine keyframes (stride + legs-passing) duplicated once each — leg-region pixel diff 117 between keyframes vs 22-28 between duplicates. Locked template: walk = 2 exaggerated keyframes (engine plays A-B-A-B, playback rate = speed), death = 4 poses (these DO come out distinct — stagger/kneel/crumple/flat worked first try). Feet baseline came back pixel-perfect in every generation (y variance ≤1px). Shipped assets: `learn/drill/assets/enemies/goblin-walk.png` (2 cells) + `goblin-death.png` (4 cells), chroma-keyed to real alpha with 1px edge erosion + despill, per-component masked packing (overlapping poses on the source sheet cross-contaminated rectangular crops — mask by connected-component label, not bbox). Watermark note: the Gemini sparkle appears on sprite sheets too (walk sheet had one on the empty magenta); it survives chroma-keying as floating pale pixels, so run the sparkle check on every sheet. |
| A-2 | Verdant enemy roster sheets | 35 | P1 | TODO | A-1 | **User + Nano.** Apply the A-1 template (as revised by the pilot: TWO maximally-contrasted walk keyframes per enemy, not 6 — see A-1 notes) to the rest of the roster: orc, raider (fast), brute (armored), wisp (flying — 2 hover keyframes instead of steps, no ground shadow baked in), shaman (healer — death poses plus 2 cast keyframes), boss (bigger canvas). Generate walk and death as SEPARATE generations, attaching the canonical character image as reference for the second one (the goblin pilot drifted between sheets until re-anchored). One sheet per enemy, one PR per batch. "Running" needs no extra frames — speed is conveyed by playback rate. **Readability (added after goblin was invisible in-game):** accent colors from the reserved palette (reds/purples/blues/bone — never terrain greens/browns), and every sheet goes through `scripts/pop_unit_sheet.py` (outline + pop) before commit — see MAP_ART_PIPELINE.md §4. |
...
*Last updated: 2026-07-05 (backlog review: W-1/W-7 marked DONE — both shipped; new K section added from a Kingdom Rush / BTD6 / GemCraft / Defense Grid benchmark pass — start with K-1/K-2/K-3, they are the highest value-per-point in the file; spine revised with Fishing Camp; ranger arrow physics rebalanced). Picking order: filter `Status = TODO`, sort by Priority then Effort ascending, take the first item whose dependencies are all `DONE` or `—`. W-1 and G-9 are flagged do-not-auto-implement.*
